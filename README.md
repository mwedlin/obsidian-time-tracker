...
- **Edit** (✎) — adjust a task's name, start time, or end time by hand. Times can be typed either in your configured timestamp format, or as natural language (e.g. "yesterday at 3pm") if you have a Natural Language Dates plugin installed — either the original [Natural Language Dates](https://github.com/argenos/nldates-obsidian), or the actively maintained fork [Natural Language Dates (Revived)](https://github.com/Amato21/nldates-revived) if the original is no longer available in Obsidian's community plugin browser.
...
## Generating a report
Place your cursor where you want the report to appear in the current note, then open the command palette and execute the `Time Tracker: Report` command. Choose a start and end date, typed as `YYYY-MM-DD` (not your configured Timestamp Display Format - this dialog only ever deals in whole calendar days, so it always uses a plain date format regardless of that setting), or as natural language if a Natural Language Dates plugin is installed — "Check dates" lets you confirm they were parsed correctly first; if a date doesn't parse, a notice explains the expected format instead of the button silently doing nothing. Hit **Append table at cursor** and a Markdown table of hours logged per project, per day, is written directly into the note at the cursor position.
...
# 🧩 Works well with
- **[Natural Language Dates](https://github.com/argenos/nldates-obsidian)** or its actively maintained fork, **[Natural Language Dates (Revived)](https://github.com/Amato21/nldates-revived)** (optional, either works) — install one to type things like "next monday" or "yesterday at 3pm" wherever this plugin asks for a date, instead of the strict timestamp format.
